import pieces  # rules and constructors for the pieces
...
class Board:
    ...
    def is_legal(self, move):
        ...
        """
        piece = move[0]
        start = piece.location  # where the move starts
        new_position = move[1]
        ...
        if not isinstance(piece, pieces.Knight) and not self.has_line_of_sight(start, new_position - start):
            return False

        # If there's a piece at end, check if it's on same team / a king (who cannot be captured directly)
        if not self.is_occupied(new_position) and (self.is_same_color(piece, new_position - start) or
                                          isinstance(self.current_state[new_position[0]][new_position[1]], pieces.King)):
            return False

        # If the piece being moved is a king, make sure move wouldn't move it into check
        if isinstance(piece, pieces.King) and self.is_checked(piece.color, new_position):
            return False

        return True

    def has_line_of_sight(self, start, position_change):
        ...
        position_change := (change in rows, change in columns)
        """
        current = list(start)  # so we can modify values
        end = start + position_change
        ...
        while True:
            ...
            if current == end:  # simulate a do-while
                break

        return True

    def is_same_color(self, piece, end):
        if not self.is_occupied(piece.position) or not self.is_occupied(end):
            return False
        return piece.color == self.current_state[end[0]][end[1]].color

    def is_checked(self, color, position):
        """Returns true if a king of the given color would be checked at that position."""
        raise NotImplementedError  # todo finish - simulate?
