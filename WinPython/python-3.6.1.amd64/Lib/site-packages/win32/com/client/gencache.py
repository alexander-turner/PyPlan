...
from win32 import types as win32types
...
is_readonly = is_zip = hasattr(win32.com, "__loader__") and hasattr(
    win32.com.__loader__, "archive")
...
def _LoadDicts():
    ...
    if is_zip:
        import io as io
        loader = win32.com.__loader__
        arc_path = loader.archive
        dicts_path = os.path.join(win32.com.__gen_path__, "dicts.dat")
        ...
        f = open(os.path.join(win32.com.__gen_path__, "dicts.dat"), "rb")
    try:
...
def GetGeneratePath():
    ...
    try:
        os.makedirs(win32.com.__gen_path__)
        # os.mkdir(win32.com.__gen_path__)
    except os.error:
        pass
    try:
        fname = os.path.join(win32.com.__gen_path__, "__init__.py")
        os.stat(fname)
    except os.error:
        f = open(fname, "w")
        f.write(
        ...
        f.write('import win32.com\n')
        f.write(
            'if __path__[:-1] != win32.com.__gen_path__: __path__.append(win32.com.__gen_path__)\n'
        )
        f.close()

    return win32.com.__gen_path__


#
# The helpers for win32.com_client.Dispatch and OCX clients.
#
...
def GetClassForCLSID(clsid):
    ...
    """
    # first, take a short-cut - we may already have generated support
    # ready-to-roll.
    clsid = str(clsid)
    ...
    if mod is not None:
        ...
        if sub_mod is not None:
            ...
            try:
                ...
                # use it (it may not be registered, causing a lookup by clsid
                # to fail)
...
def MakeModuleForTypelib(typelibCLSID,
                         lcid,
                         major,
                         minor,
    ...
    if bGUIProgress is not None:
        print(
            "The 'bGuiProgress' param to 'MakeModuleForTypelib' is obsolete.")
...
def MakeModuleForTypelibInterface(typelib_ob,
                                  progressInstance=None,
                                  bForDemand=bForDemandDefault,
                                  bBuildHidden=1):
    """Generate support for a type library.
...
def EnsureModuleForTypelibInterface(typelib_ob,
                                    progressInstance=None,
                                    bForDemand=bForDemandDefault,
                                    bBuildHidden=1):
    ...
    if bForDemand:
        demandGeneratedTypeLibraries[(str(guid), lcid, major,
                                      minor)] = typelib_ob
    ...
    try:
        ...
        pass
    # Generate it.
    return MakeModuleForTypelibInterface(typelib_ob, progressInstance,
...
def EnsureModule(typelibCLSID,
    ...
    try:
        try:
            ...
            module = None
            try:
                tlbAttr = win32.com.LoadRegTypeLib(typelibCLSID, major, minor,
                                                   lcid).GetLibAttr()
                # if the above line doesn't throw a win32.com.com_error, check if
                # it is actually a different lib than we requested, and if so,
                # suck it in
                if tlbAttr[1] != lcid or tlbAttr[4] != minor:
                    # print "Trying 2nd minor #", tlbAttr[1], tlbAttr[3],
                    # tlbAttr[4]
                    try:
        ...
        if module is not None and bValidateFile:
            ...
            try:
                typLibPath = win32.com.QueryPathOfRegTypeLib(
                ...
                tlbAttributes = win32.com.LoadRegTypeLib(
    ...
    if module is None:
        ...
        if is_readonly:
            ...
            if items:
                ...
                # We want the latest minor version we have - so just sort and
                # grab last
                items.sort()
                new_minor = items[-1][3]
...
def EnsureDispatch(prog_id, bForDemand=1):
    ...
    disp = win32.com.client.Dispatch(prog_id)
...
def AddModuleToCache(typelibclsid,
                     lcid,
                     major,
                     minor,
                     verbose=1,
...
def _GetModule(fname):
    ...
    """
    mod_name = "win32.com.gen_py.%s" % fname
...
def _Dump():
    print("Cache is in directory", win32.com.__gen_path__)
    # Build a unique dir
    d = {}
