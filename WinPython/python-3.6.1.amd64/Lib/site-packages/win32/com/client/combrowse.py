...
class HLICategory(HLICOM):
    ...
    def GetSubList(self):
        ...
        catinf = pythoncom.CoCreateInstance(
            ...
            pythoncom.IID_ICatInformation)
        ret = []
        for clsid in util.Enumerator(
                catinf.EnumClassesOfCategories((catid,), ())):
...
class HLIRegisteredTypeLibrary(HLICOM):
    ...
    def GetSubList(self):
        ...
        key = win32api.RegOpenKey(
            win32con.HKEY_CLASSES_ROOT, "TypeLib\\%s\\%s" %
            (clsidstr, versionStr))
        win32ui.DoWaitCursor(1)
        try:
            num = 0
...
class HLITypeLibFunction(HLICOM):
    ...
    def MakeReturnType(self, returnTypeDesc):
        if isinstance(returnTypeDesc, type(())):
