...
if __name__ == '__main__':
    ...
    simulate.run(initial_state=connect4, agents_list=[u_ro, nested_u_ro], num_trials=1, verbose=False)

    e_ro = e_rollout_agent.ERolloutAgentClass(depth=10, num_pulls=100, epsilon=0.5, policy=rand_agent)

    nested_e_ro = e_rollout_agent.ERolloutAgentClass(depth=10, num_pulls=10, epsilon=0.5, policy=e_ro)

    connect4.initialize()  # reset the state
    simulate.run(initial_state=connect4, agents_list=[u_ro, e_ro], num_trials=10, verbose=False)

    connect4.initialize()  # reset the state
    simulate.run(initial_state=connect4, agents_list=[nested_u_ro, nested_e_ro], num_trials=10, verbose=False)
